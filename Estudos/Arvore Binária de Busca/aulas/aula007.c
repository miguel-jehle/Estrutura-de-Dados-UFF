/*
    FINALIZANDO ALGORTIMO DE REMOÇÃO!!!

    ALGORITMO: Ao encontramos o nó que desejamos remover, caso ele tenha 2 filhos, iremos criar um nó auxiliar que apontará para o seu filho da esquerda,
               e então iremos andar o mais a direita possível (podemos primeiro ir para direita e depois o mais esquerda possível, fica a sua escolha).
               Com o mais a direita encontrada, trocaremos o valor deste com o do nosso ponteiro raiz, que está com o nó que desejamos apagar. Assim que
               realizar a troca, a árvore momentaneamente estará incorreta, para conseguirmos encontrar sem problemas, vamos chamar a função de remover para
               o filho da esquerda na qual nosso ponteiro raiz está. Com isso encontraremos o valor, mas agora, em uma posição mais simples (folha ou 1 filho),
               e então a função irá remove-lo usando o próprio algoritmo por se tratar de uma chamada recursiva.Por fim retornarmos a raiz.
               
                Assim temos nossa função de remoção completa!!!! Espero que o eu do futuro entenda!!! Fiz com carinho pra você <3, não surte com a prova!!!
*/
        ...
        if (raiz->valor == num){ //Caso encontremos, vamos ver se ele é folha ou não
            if (!raiz->direita && !raiz->esquerda){ //Se for folha, basta dar free e retornar NULL
                ...
                if (raiz->direita && raiz->esquerda){ //Se tiver 2 filhos, faremos:
                    ...
                    //O algortimo foi aprendido em uma video-aula e está descrito no começo do código
                    
                    ArvNo *aux = raiz ->esquerda; //Aponto um ponteiro para o Nó a esquerda do que desejo remover
                    while (aux->direita) aux = aux->direita; //E então ando o mais a direita possível
                    //Ao encontrar o mais a direita, troco o valor dele, com o nó que desejo retirar
                    raiz->valor = aux->valor;
                    aux->valor = num;
                    //Com a troca feita, podemos chamar a função remove para o nó a esquerda do que está o ponteiro principal
