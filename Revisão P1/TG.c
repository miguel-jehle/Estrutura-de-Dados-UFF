...
void TG_imp_rec(TG *g){
  if(g){
    if(g->id_no == INT_MIN) printf("\n\tfonte : ");
    if(g->id_no == INT_MAX) printf("\n\tsumidouro : ");
    if((g->id_no != INT_MIN) && (g->id_no != INT_MAX)) printf("\n\t%d: ", g->id_no);
    TVIZ *v = g->prim_viz;
    while(v){
      if(v->id_viz == INT_MIN) printf(" fonte --> ");
      if(v->id_viz == INT_MAX) printf(" sumidouro --> ");
      if((v->id_viz != INT_MIN) && (v->id_viz != INT_MAX)) printf(" %d -->", v->id_viz);
      v = v->prox_viz;
    }
    printf("\n");
